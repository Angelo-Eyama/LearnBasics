...
@router.get("/comments/", response_model=List[CommentRead], tags=["Comentarios"])
def get_comments(session: Session = Depends(get_session)):
    comments = comments_controller.get_comments(session)
    return comments

@router.get("/comments/{comment_id}", response_model=CommentRead, tags=["Comentarios"])
...
@router.get("/comments/user/{user_id}", response_model=List[CommentRead], tags=["Comentarios"])
...
@router.post("/comments/", response_model=CommentRead, tags=["Comentarios"])
...
@router.patch("/comments/{comment_id}", response_model=CommentRead, tags=["Comentarios"])
...
@router.delete("/comments/{comment_id}", response_model=CommentRead, tags=["Comentarios"], summary="Eliminar un comentario", description="Elimina un comentario del sistema utilizando su ID.", response_description="El comentario eliminado.")
