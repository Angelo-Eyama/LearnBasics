...
@router.get("/submissions/", response_model=List[SubmissionRead], tags=["Entregas"])
def get_submissions(session: Session = Depends(get_session)):
    submissions = submissions_controller.get_submissions(session)
    return submissions

@router.get("/submissions/{submission_id}", response_model=SubmissionRead, tags=["Entregas"])
...
@router.post("/submissions/", response_model=SubmissionCreate, tags=["Entregas"])
...
@router.patch("/submissions/{submission_id}", response_model=SubmissionUpdate, tags=["Entregas"])
